...
def set_train_sampler_loader(g, k, args):
...
def cal_center(features, labels):
    lbs_unqs = np.unique(labels)
    centers = features[0,:]

    for i in lbs_unqs:
        idx = np.array(np.where(labels == i))
        idx = np.reshape(idx, [idx.shape[1]])
        fea_i = features[idx, :]
        fea_ave = np.average(fea_i,axis=0)
        centers = np.vstack([centers,fea_ave])
