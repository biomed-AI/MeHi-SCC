...
class load_data_origin_data(Dataset):
    def __init__(self, dataset, load_type="csv", take_log=False, scaling=False):
        ...
        def load_csv():
            ...
            self.b = dataset_list[0]['sample_labels'].astype(np.int32)
...
if __name__ == "__main__":
    ...
    file_path = "/home/panzx/GraphSCC/data/" + args.name + ".csv"
    ...
    train_idx, test_idx = random_split(range(cell_num), [round(args.train_ratio * cell_num), round(args.test_ratio * cell_num)],
                                       generator=torch.Generator().manual_seed(args.seed))
    train_idx = train_idx.indices
    test_idx = test_idx.indices

    f = open('/home/panzx/Hi-LANDER/data/' + args.name + '_full.pkl', 'wb')
    ...
    f = open('/home/panzx/Hi-LANDER/data/' + args.name + '_train.pkl', 'wb')
    pickle.dump([features_train, labels_train, batch_train], f)
    f.close()
    ...
    f = open('/home/panzx/Hi-LANDER/data/' + args.name + '_test.pkl', 'wb')
    pickle.dump([features_test, labels_test, batch_test], f)
    f.close()